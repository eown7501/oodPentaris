...
public class GameBoardSolo implements Runnable {
	...
	/** GameBoard 를 초기화 합니다. */
	public void initGameBoardSolo() { // 게임보드 초기상태 설정
		start = true;
		score = 0;
		level = 1;
	...
	public void setNextBlock() {
		...
		for (int i = 0; i < nextBlock.coordSolo.length; i++)
			NextBlockBoard[nextBlock.coordSolo[i].getX() + 2][nextBlock.coordSolo[i].getY() + 2] = 2;
	}
	 ...
	 *            - Block의 종류 입니다.
	 */
	public void changePointSolo(Point position, int value) {
	 ...
	 */
	public boolean isCollisionSolo(Point position) {
	 ...
	 */
	public boolean isCollistionSpinSolo(Point position) {
		if (position.getX() > ROWS - 1)
			return true;
		if (position.getX() < 0)
			return true;
